...
const createMultiTenantStructure = async () => {
  try {
    ...
    // 2. Add account_id to users table (if column doesn't exist)
    await pool.query(`
      DO $$ 
      BEGIN 
    ...
    // 3. Add account_id to properties table (if column doesn't exist)
    await pool.query(`
      DO $$ 
      BEGIN 
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                      WHERE table_name='properties' AND column_name='account_id') THEN
          ALTER TABLE properties ADD COLUMN account_id INTEGER NOT NULL DEFAULT 1 REFERENCES accounts(id) ON DELETE CASCADE;
        END IF;
      END $$;
    `);

    // 4. Add account_id to contacts table (if column doesn't exist)
    await pool.query(`
      DO $$ 
      BEGIN 
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                      WHERE table_name='contacts' AND column_name='account_id') THEN
          ALTER TABLE contacts ADD COLUMN account_id INTEGER NOT NULL DEFAULT 1 REFERENCES accounts(id) ON DELETE CASCADE;
        END IF;
      END $$;
    `);

    // 5. Create indexes for account_id fields
    ...
    // 6. Create default demo account if none exist
    ...
    if (accountCount === 0) {
      ...
      // Update existing users to belong to demo account
      await pool.query(`
        UPDATE users 
        SET account_id = $1 
    ...
    // 7. Enable RLS on all tenant-specific tables
    ...
    // 8. Drop existing policies if they exist (for re-running migration)
    ...
    // 10. Create RLS policies for contacts
    await pool.query(`
    ...
    // 11. Create RLS policies for contact_service_locations (via contacts and properties)
    await pool.query(`
    ...
    // 12. Create database roles and grants
